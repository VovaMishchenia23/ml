...
def separate_data():
    ...
    df_weather_event_isw = df_weather_event_isw.drop(
        ["city", "region", "region_city", "hour_level_event_time",
         "hour_datetime", "region_id_x", "hour_conditions"], axis=1)
    ...
    X_train.to_csv(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{X_TRAIN}", index=False, encoding="utf-8")
    X_test.to_csv(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{X_TEST}", index=False, encoding="utf-8")
    y_test.to_csv(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{Y_TEST}", index=False, encoding="utf-8")
    y_train.to_csv(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{Y_TRAIN}", index=False, encoding="utf-8")
    with open(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{X_TEST_TFIDF_MATRIX}", "wb") as file:
        pickle.dump(X_test_tf_idf, file)
    with open(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{X_TRAIN_TFIDF_MATRIX}", "wb") as file:
        pickle.dump(X_train_tf_idf, file)


def create_matrix():
    separate_data()
    merge_matrix(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{X_TEST}", f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{X_TEST_TFIDF_MATRIX}",
                 f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{WORK_TEST_MATRIX}")
    merge_matrix(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{X_TRAIN}",
                 f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{X_TRAIN_TFIDF_MATRIX}",
                 f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{WORK_TRAIN_MATRIX}")
    load_y()


def load_y():
    y_test = pd.read_csv(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{Y_TEST}")
    y_test = scipy.sparse.csr_matrix(y_test.values)
    print(y_test.shape)
    with open(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{WORK_Y_TEST}", "wb") as file:
        pickle.dump(y_test, file)

    y_train = pd.read_csv(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{Y_TRAIN}")
    y_train = scipy.sparse.csr_matrix(y_train.values)
    print(y_test.shape)
    with open(f"{DATA_FOLDER2}{FINAL_DATA_FOLDER}{WORK_Y_TRAIN}", "wb") as file:
